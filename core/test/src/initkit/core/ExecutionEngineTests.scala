...
object ExecutionEngineTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    ...
    test("emits skip events for condition-skipped and state-completed entries"):
      ...
      assert(result.events.collect { case PlanEvent.Skipped(_, reasons, _) =>
        reasons
      }.flatten.contains("already completed in state"))
      assert(
        result.events
      ...
      assert(result.state.entries.find(_.name.contains("fedora-only")).exists(_.status ==
    ...
    test("dry-run emits operation data without completing or writing state"):
      ...
      assert(result.events.collect { case PlanEvent.DryRunOperation(_, data, _) => data } ==
        Vector(dryRunData))
      assert(ExecutionState.completedNames(result.state).isEmpty)
      assert(stateWriter.writes.isEmpty)

    test(
      "interrupt writes configured state, emits resume instructions, and returns configured exit code"
    ):
      ...
      assert(result.result.interrupted.flatMap(_.instructions) ==
        Vector("Log out", "Run initkit apply again", "Paused"))
  ...
  private val fixedInstant: Instant = Instant.parse("2026-06-28T12:00:00Z")

  private val laterInstant: Instant = Instant.parse("2026-06-28T12:05:00Z")

  private val fixedClock: Clock = Clock.fixed(fixedInstant, ZoneOffset.UTC)

  private val defaultStatePath: Path = Paths.get("/tmp/initkit-state.json")
  ...
  private def interruptEntry(name: String, statePath: String): PlanEntry = PlanEntry(
    name = Some(name),
    kind = Some("interrupt"),
    description = None,
    execution = Some(Execution(
  ...
  private final class RecordingInstaller(
    ...
    def installedNames: Vector[String] = installedNamesRef.get()

    override def installApt(
        operation: PackagePlanOperation[PackageSpec.Apt],
        policy: ExecutionPolicy
    ): PlanOperationOutcome = record(operation.summary)
    ...
    private def record(operation: PlanOperationSummary): PlanOperationOutcome =
      ...
      outcomes.getOrElse(
        operation.name,
        PlanOperationOutcome.Completed(Vector(s"completed ${operation.name}"))
      )
  ...
  private final class RecordingStateWriter extends ExecutionStateWriter:
    ...
    def writes: Vector[(Path, ExecutionState)] = writesRef.get()
