...
object PackageManagerInstallersTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    ...
    test("dry-run returns command previews without executing package commands"):
      ...
      assert(dryRun.actions ==
        perPackage(Vector("dnf", "install", "-y"), baseDnfPackages).map(dryRunCommand))

    test("apply mode runs package commands through the command executor"):
      val operation = planOperation("snap-desktop-apps")
      val commands  = PackageManagerInstallers.commandSpecs(
        packagePlanOperation("snap-desktop-apps"),
        applyPolicy
      )
    ...
    test("apply mode attempts later package commands after an item failure"):
      val operation = planOperation("dnf-base-cli")
      val commands  =
        PackageManagerInstallers.commandSpecs(packagePlanOperation("dnf-base-cli"), applyPolicy)
      val executor = FakeCommandExecutor(
    ...
    test("sudo is disabled when requireSudo is false"):
      val commands =
        packageCommands("flatpak-desktop-apps", policy = applyPolicy.copy(requireSudo = false))
  ...
  private val baseZypperPackages: Vector[String] = Vector(
  ...
  ): Vector[CommandSpec] = PackageManagerInstallers.commandSpecs(packagePlanOperation(name), policy)
  ...
  ): Vector[Vector[String]] = packageNames.map(packageName => prefix :+ packageName)

  private def dryRunCommand(argv: Vector[String]): DryRunAction = DryRunAction.Command(
    argv = argv,
    shell = None,
    sudo = true,
