  ...
  val tests: Tests = Tests:
    test("apply mode runs source setup commands through the command executor"):
      ...
      val outcome = SourceSetupExecutor(executor, files).execute(
        SourceSetupPlan(Vector(SourceSetupOperation.RunCommand("Add source", command)), Vector.empty, aptUpdateBeforeInstall = false),
        applyPolicy,
        sourceSetupSummary
      )

      assert(outcome == PlanOperationOutcome.Completed(Vector("ran source setup command 'Add source'")))
    ...
    test("apply mode reports source setup command failures"):
      ...
      val outcome = SourceSetupExecutor(executor, RecordingSourceSetupFiles()).execute(
        SourceSetupPlan(Vector(SourceSetupOperation.RunCommand("Add failing source", command)), Vector.empty, aptUpdateBeforeInstall = false),
        applyPolicy,
        sourceSetupSummary
      )
    ...
    test("apply mode writes source files with parent directory creation and configured mode"):
      ...
      try
        ...
        assert(Files.readString(destination, StandardCharsets.UTF_8) == "deb https://example.test stable main\n")
    ...
    test("apply mode reports source file write failures"):
      ...
      val outcome = SourceSetupExecutor(FakeCommandExecutor(Vector.empty), files).execute(
        SourceSetupPlan(
          Vector(SourceSetupOperation.WriteFile("Write failing source", destination, "content", Some("0644"), sudo = false)),
          Vector.empty,
      ...
      assert(files.writes == Vector(SourceSetupFileWrite(destination, "content", Some("0644"), sudo = false)))
      assert(failure.exitCode.isEmpty)
      assert(failure.message == "source setup file write 'Write failing source' failed for /etc/initkit/failing-source.list: permission denied")
    ...
    test("sudo-marked source file writes are passed to the file boundary and reported"):
      ...
      val outcome = SourceSetupExecutor(FakeCommandExecutor(Vector.empty), files).execute(
        SourceSetupPlan(
          Vector(SourceSetupOperation.WriteFile("Write sudo source", destination, "content", Some("0644"), sudo = true)),
          Vector.empty,
          aptUpdateBeforeInstall = false
        ),
        applyPolicy,
      ...
      assert(files.writes == Vector(SourceSetupFileWrite(destination, "content", Some("0644"), sudo = true)))
      assert(outcome == PlanOperationOutcome.Completed(
        Vector("wrote source setup file 'Write sudo source' to /etc/apt/sources.list.d/docker.list mode=0644 (sudo requested)")
      ))
    ...
    test("dry-run returns source setup previews without running commands or writing files"):
      ...
      val plan = SourceSetupPlan(
        Vector(
          ...
          SourceSetupOperation.WriteFile("Preview file", destination, "content", Some("0644"), sudo = true)
        ),
        Vector.empty,
      ...
      val files = RecordingSourceSetupFiles()
      val outcome = SourceSetupExecutor(executor, files).execute(plan, dryRunPolicy, sourceSetupSummary)
...
private final class RecordingSourceSetupFiles(
  ...
  def writes: Vector[SourceSetupFileWrite] =
    writesRef.get()
