...
object DotfilesExecutorTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    test("command generation includes clone update checkout preview and apply"):
      ...
      assert(argvs(commands) == Vector(
        Vector(
          "git",
          "clone",
          "https://github.com/w0rxbend/system-bootstrap.git",
          "${dotfilesDir}"
        ),
      ...
      assert(DotfilesExecutor.setupCommandSpecs(
        operation.spec.copy(repository = operation.spec.repository.copy(update = Some(false))),
        includeClone = true
      ).map(argv) == Vector(
        Vector(
          "git",
        ...
        assert(dryRun.actions == Vector(
          DryRunAction.Command(
            Vector(
              "git",
              "clone",
              operation.spec.repository.url,
              operation.spec.repository.destination
            ),
            None,
            false,
            None
          ...
          DryRunAction.Command(
            Vector("git", "-C", operation.spec.repository.destination, "pull", "--ff-only"),
            None,
            false,
            None
          ),
          DryRunAction.Command(
            Vector("git", "-C", operation.spec.repository.destination, "checkout", "main"),
            None,
            false,
            None
          ),
    ...
    test(
      "apply clones missing repo then updates checkouts verifies config and previews before apply"
    ):
      withTempDir: tempDir =>
        ...
        assert(outcome == PlanOperationOutcome.Completed(
          Vector("ran 3 dotfiles repository commands; ran 2 dotfiles apply commands")
        ))
  ...
  extension (repository: GitRepository)
    private def destinationPath: Path = Path.of(repository.destination)
  ...
  private def tempOperation(
      tempDir: Path,
      createConfig: Boolean
  ): InstallerPlanOperation[InstallerSpec.DotfilesApply] =
    ...
    dotfilesOperation("apply-dotfiles").copy(
      spec = InstallerSpec.DotfilesApply(
        tool = ToolInvocation(
          "/tmp/dotbot-go",
          Vector("-d", destination.toString, "-c", configPath.toString)
        ),
  ...
  private def argvs(commands: Vector[CommandSpec]): Vector[Vector[String]] = commands.map(argv)

  private def argv(command: CommandSpec): Vector[String] = command.invocation match
