...
object ExecutionStateTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    ...
    test("stale state rejects mismatched manifest name and fingerprint without reset"):
      withTempDir: tmp =>
        ...
        val changedFingerprint =
          manifest("developer-workstation", entries = Vector("first", "second", "third"))

        ExecutionStateStore.write(statePath, baseState)

        val nameMismatch = ExecutionStateStore.loadOrInitialize(
          statePath,
          changedName,
          resetState = false,
          fixedClock
        )
        val fingerprintMismatch = ExecutionStateStore.loadOrInitialize(
          statePath,
          changedFingerprint,
          resetState = false,
          fixedClock
        ...
        assert(
          nameMismatch.left.toOption.exists(_.isInstanceOf[ExecutionStateError.StaleManifestName])
        )
        assert(fingerprintMismatch.left.toOption.exists(
          _.isInstanceOf[ExecutionStateError.StaleManifestFingerprint]
        ))
    ...
    test("reset state ignores stale file and returns a fresh state for the current manifest"):
      withTempDir: tmp =>
        ...
        val resetState = ExecutionStateStore.loadOrInitialize(
          statePath,
          currentManifest,
          resetState = true,
          fixedClock
  ...
  private val fixedInstant: Instant = Instant.parse("2026-06-28T10:15:30Z")

  private val laterInstant: Instant = Instant.parse("2026-06-28T10:20:30Z")
  ...
  private def planEntry(name: String): PlanEntry = PlanEntry(
    name = Some(name),
    kind = Some("commands"),
    description = None,
