...
object ExecutionContractsTests extends TestSuite:
    ...
    test("builds plan result counts from terminal events and remaining operations"):
      ...
      val result = PlanResult.fromEvents(
        events = Vector(
          ...
          PlanEvent.Skipped(
            skipped,
            Vector("matched --skip selector 'apt-packages'"),
            fixedInstant
          ),
      ...
      assert(result.counts ==
        PlanResultCounts(completed = 1, skipped = 1, failed = 1, interrupted = 1, remaining = 1))
      assert(result.completed == Vector(completed))
      assert(result.skipped ==
        Vector(PlanSkip(skipped, Vector("matched --skip selector 'apt-packages'"))))
  ...
  private object RecordingInstaller extends PlanOperationInstaller:
    ...
    override def installPacman(
        ...
        policy: ExecutionPolicy
    ): PlanOperationOutcome = unusedPackage(operation)
    ...
    override def installBinaryDownloads(
        ...
        policy: ExecutionPolicy
    ): PlanOperationOutcome = unusedInstaller(operation)
    ...
    private def unusedInstaller(operation: InstallerPlanOperation[InstallerSpec])
        : PlanOperationOutcome =
      PlanOperationOutcome.Completed(Vector(s"unused:${operation.summary.name}"))
  ...
  private def entry(name: String, kind: String, spec: RawYaml): PlanEntry = PlanEntry(
    name = Some(name),
    kind = Some(kind),
    description = None,
