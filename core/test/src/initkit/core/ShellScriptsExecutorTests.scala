...
object ShellScriptsExecutorTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    test("renders rustup and miniforge download and execution operations from config example"):
      ...
      assert(commandArgv(commands(0)) ==
        Vector("sh", "-s", "--", "-y", "--default-toolchain", "stable"))
      assert(commands(0).stdinFile == Some(rustupTemp))
      assert(commandArgv(commands(1)) ==
        Vector("bash", miniforgeTemp.toString, "-b", "-p", "${home}/miniforge3"))
    ...
    test("creates paths skip duplicate shell script execution"):
      ...
      assert(outcome ==
        PlanOperationOutcome.Completed(Vector("ran 1 shell script(s), skipped 1 shell script(s)")))
    ...
    test("configured shell and args preserve argv boundaries"):
      ...
      assert(commandArgv(command) ==
        Vector("bash", "/tmp/custom.sh", "--flag=value with spaces", "--", "literal * value"))
    ...
    test("dry-run previews downloads and execution without mutating files"):
      ...
      assert(dryRun.actions == Vector(
        DryRunAction.Message(
          "download shell script 'rustup' from https://sh.rustup.rs to /preview/rustup.sh"
        ),
  ...
  private val rustupTemp: Path = Path.of("/tmp/initkit-rustup.sh")

  private val miniforgeTemp: Path = Path.of("/tmp/initkit-miniforge.sh")

  private val rustupPreview: Path = Path.of("/preview/rustup.sh")

  private val miniforgePreview: Path = Path.of("/preview/miniforge.sh")
  ...
  private def shellScriptsOperation(name: String)
      : InstallerPlanOperation[InstallerSpec.ShellScripts] =
  ...
  ): Vector[CommandSpec] = operation.spec.items.zip(tempPaths).map: (item, tempPath) =>
    ShellScriptsExecutor.commandSpec(item, tempPath)

  private def miniforgeItem(operation: InstallerPlanOperation[InstallerSpec.ShellScripts])
      : ShellScriptItem = operation.spec.items
...
private final class RecordingShellScriptDownloader private (
  ...
  def calls: Vector[ShellScriptDownloadCall] = callsRef.get()
...
private object RecordingShellScriptDownloader:
  def successful: RecordingShellScriptDownloader = new RecordingShellScriptDownloader(Right(()))
...
private final class RecordingShellScriptFiles(
  ...
  def created: Vector[Path] = stateRef.get().created

  override def exists(path: Path): Boolean = existing.contains(path)
  ...
  override def createTempScript(itemName: String): Either[ShellScriptFileError, Path] =
    ...
    state.remainingTempPaths.headOption match
      case Some(path) =>
        ...
        Right(path)
      case None => Left(ShellScriptFileError(s"no temp path configured for $itemName"))
  ...
  override def previewTempScriptPath(itemName: String): Path =
    ...
    state.remainingPreviewPaths.headOption match
      case Some(path) =>
        ...
        path
      case None => throw new java.lang.AssertionError(s"no preview path configured for $itemName")
