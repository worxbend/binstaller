...
object ExecutionWithSourceSetupTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    test("apply mode runs source setup before selected package commands"):
      ...
      val result =
        runWithSourceSetup(testManifest, state, executor, writer, sourceCommand, applyPolicy)

      assert(result.exitCode == 0)
      assert(executor.calls == Vector(sourceCommand, packageCommand))
      assert(result.events.collect { case PlanEvent.Completed(operation, _, _) =>
        operation.name
      } == Vector("source-setup", "apt-base-cli"))
    ...
    test("source setup failure stops package execution"):
      ...
      val executor      = FakeCommandExecutor(
        Vector(FakeCommandResponse(
          sourceCommand,
          CommandResultData.exited(9, duration = Duration.Zero)
        ))
      )
      val writer = RecordingStateWriter()
    ...
    test("dry-run previews source setup and packages without commands or state writes"):
      ...
      val result =
        runWithSourceSetup(testManifest, state, executor, writer, sourceCommand, dryRunPolicy)

      assert(result.exitCode == 0)
      assert(executor.calls.isEmpty)
      assert(writer.writes.isEmpty)
      assert(result.events.collect { case PlanEvent.DryRunOperation(operation, _, _) =>
        operation.name
      } == Vector("source-setup", "apt-base-cli"))
  ...
  private val defaultStatePath: Path = Path.of("/tmp/initkit-source-setup-state.json")
  ...
  private def packageCommands(testManifest: Manifest): Vector[CommandSpec] =
    PlanOperation.decode(0, testManifest.spec.plan.head) match
      ...
      case Right(other) => fail(s"expected apt operation, got $other")
