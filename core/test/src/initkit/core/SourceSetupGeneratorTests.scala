...
object SourceSetupGeneratorTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    test("generates apt source setup and apt update marker for apt hosts"):
      ...
      val aptKey = plan.operations.collectFirst {
        case operation @ SourceSetupOperation.RunCommand(_, _) => operation
      }.get
      val aptFile = plan.operations.collectFirst {
        case operation @ SourceSetupOperation.WriteFile(_, _, _, _, _) => operation
      }.get
      ...
      assert(aptFile.content ==
        "deb [arch=amd64] https://download.docker.com/linux/ubuntu noble stable\n")
      assert(dryRun.actions.exists(_ == DryRunAction.FileWrite(
        "/etc/apt/sources.list.d/docker.list",
        Some("0644"),
        "package source configuration"
      )))
      assert(dryRun.actions.exists(_ == DryRunAction.Message(
    ...
    test("skips apt and zypper source sections for dnf hosts"):
      ...
      val dnfFile = plan.operations.collectFirst {
        case operation @ SourceSetupOperation.WriteFile(_, _, _, _, _) => operation
      }.get
    ...
    test("generates zypper addrepo commands only for zypper hosts"):
      val plan =
        SourceSetupGenerator.generate(Some(allSources.copy(flatpak = None)), zypperHost, sudoPolicy)
    ...
    test("flatpak remote setup uses if-not-exists when requested"):
      val plan = SourceSetupGenerator.generate(
        Some(Sources(None, None, None, allSources.flatpak, raw = RawYaml.NullValue)),
        aptHost,
        sudoPolicy
      )
      ...
      assert(plan.operations.size == 1)
      assert(commandArgv(plan.operations.head) == Vector(
        "flatpak",
        "remote-add",
        "--if-not-exists",
        "flathub",
    ...
    test("dry-run data exposes source setup commands"):
      val plan = SourceSetupGenerator.generate(
        Some(allSources.copy(apt = None, dnf = None, zypper = None)),
        aptHost,
        sudoPolicy
      )
      val actions = plan.dryRunData(sourceSetupSummary).actions

      assert(actions == Vector(DryRunAction.Command(
        Vector(
          "flatpak",
          "remote-add",
          "--if-not-exists",
  ...
  private val sudoPolicy: ExecutionPolicy = ExecutionPolicy(
  ...
  private val allSources: Sources = Sources(
    apt = Some(
      AptSources(
        repositories = Vector(
          AptRepository(
  ...
  private def commandArgv(operation: SourceSetupOperation): Vector[String] = operation match
    case SourceSetupOperation.RunCommand(_, command) => command.redacted.invocation match
        case RedactedCommandInvocation.Direct(argv)          => argv
        case RedactedCommandInvocation.Shell(command, shell) => shell :+ command
    case other => throw new java.lang.AssertionError(s"expected command operation, got $other")
