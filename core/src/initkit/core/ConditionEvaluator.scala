...
object ConditionEvaluator:
  ...
  def evaluate(condition: Condition, hostFacts: HostFacts): ConditionEvaluation =
    val reasons = evaluateOs(condition, hostFacts) ++
  ...
  private def evaluateCommandExists(
      condition: Condition,
      hostFacts: HostFacts
  ): Vector[ConditionSkipReason] = condition.commandExists.toVector.flatMap: command =>
  ...
  ): Option[ConditionSkipReason] = Option.when(!matches(expected, actual)):
    ConditionSkipReason.OsMismatch(
      field,
      ConditionExpectation.fromMatchExpression(expected),
      actual
    )

  private def matches(expected: MatchExpression, actual: Option[String]): Boolean = actual.exists:
    value =>
      expected match
        case MatchExpression.Exact(expectedValue) => normalize(value) == normalize(expectedValue)
        case MatchExpression.OneOf(values)        =>
          values.exists(expectedValue => normalize(value) == normalize(expectedValue))

  private def normalize(value: String): String = value.trim.toLowerCase

final case class ConditionEvaluation(
    matched: Boolean,
    skipReasons: Vector[ConditionSkipReason]
):
  def userFacingSkipReasons: Vector[String] = skipReasons.map(_.message)

object ConditionEvaluation:
  val matched: ConditionEvaluation = ConditionEvaluation(matched = true, skipReasons = Vector.empty)
...
enum ConditionSkipReason:
  ...
  def message: String = this match
    ...
    case MissingCommand(command) => s"required command '$command' is not available"
...
enum ConditionExpectation:
  ...
  def description: String = this match
    case Exact(value)  => s"'$value'"
    case OneOf(values) => values.map(value => s"'$value'").mkString("one of ", ", ", "")

object ConditionExpectation:

  def fromMatchExpression(expression: MatchExpression): ConditionExpectation = expression match
