...
final class NerdFontsExecutor(
  ...
  def install(
    ...
    case ExecutionRunMode.Apply => applyFonts(operation)

  private def applyFonts(
      operation: InstallerPlanOperation[InstallerSpec.NerdFonts]
  ): PlanOperationOutcome = writeConfigIfRequested(operation.spec.config) match
    case Left(failure)        => failed(operation.summary, failure)
    case Right(configWritten) => runCommands(operation) match
        case Left(failure)  => failed(operation.summary, failure)
        case Right(results) =>
    ...
    if config.create.contains(true) then
      config.content match
        case Some(content) =>
          ...
          files.writeConfig(path, rendered).left.map(error =>
            NerdFontsFailure.Config(path, error.message)
          ).map(_ => true)
        case None => Left(NerdFontsFailure.Config(
            Path.of(config.path),
            "config.create is true but config.content is missing"
          ))
    else Right(false)
  ...
  private def failed(
      ...
      failure: NerdFontsFailure
  ): PlanOperationOutcome = PlanOperationOutcome.Failed(
    PlanFailure(
      operation = summary,
...
private enum NerdFontsFailure:
  ...
  def message: String = this match
    case Config(path, detail) => s"Nerd Fonts config write failed for $path: $detail"
    case Command(result)      =>
...
object NerdFontsFiles:
  ...
  val Jvm: NerdFontsFiles = new NerdFontsFiles:
    override def writeConfig(path: Path, content: String): Either[NerdFontsFileError, Unit] =
      try
        ...
        case error: IOException       => Left(NerdFontsFileError(error.getMessage))
        case error: SecurityException => Left(NerdFontsFileError(error.getMessage))

object NerdFontsExecutor:

  def commandSpecs(operation: InstallerPlanOperation[InstallerSpec.NerdFonts])
      : Vector[CommandSpec] = previewCommandSpec(operation.spec).toVector :+
    applyCommandSpec(operation.spec)
  ...
  def dryRunData(operation: InstallerPlanOperation[InstallerSpec.NerdFonts]): DryRunOperationData =
    DryRunOperationData(
      ...
      actions = configDryRunActions(operation.spec.config) ++
        commandSpecs(operation).map(dryRunCommand)
    )

  def renderConfig(content: RawYaml): String = renderBlock(content, indent = 0).mkString("\n") +
    "\n"
  ...
  private def configDryRunActions(config: GeneratedConfig): Vector[DryRunAction] =
    if config.create.contains(true) then
      Vector(DryRunAction.FileWrite(
        config.path,
        mode = None,
        description = "generate Nerd Fonts config"
      ))
    else Vector.empty
  ...
  private def renderBlock(value: RawYaml, indent: Int): Vector[String] = value match
    case RawYaml.MappingValue(fields) if fields.isEmpty => Vector(spaces(indent) + "{}")
    case RawYaml.MappingValue(fields)                   =>
      fields.toVector.flatMap((key, child) => renderMappingEntry(key, child, indent))
    case RawYaml.SequenceValue(items) if items.isEmpty => Vector(spaces(indent) + "[]")
    case RawYaml.SequenceValue(items) => items.flatMap(item => renderSequenceItem(item, indent))
    case scalar                       => Vector(spaces(indent) + renderScalar(scalar))

  private def renderMappingEntry(key: String, value: RawYaml, indent: Int): Vector[String] =
    value match
      case scalar if isScalar(scalar) => Vector(s"${spaces(indent)}$key: ${renderScalar(scalar)}")
      case nested => Vector(s"${spaces(indent)}$key:") ++ renderBlock(nested, indent + 2)

  private def renderSequenceItem(value: RawYaml, indent: Int): Vector[String] = value match
    case scalar if isScalar(scalar) => Vector(s"${spaces(indent)}- ${renderScalar(scalar)}")
    case nested => Vector(s"${spaces(indent)}-") ++ renderBlock(nested, indent + 2)

  private def isScalar(value: RawYaml): Boolean = value match
    case RawYaml.MappingValue(_)  => false
    case RawYaml.SequenceValue(_) => false
    case _                        => true

  private def renderScalar(value: RawYaml): String = value match
