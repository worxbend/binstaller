...
object ManifestVariableResolver:
  ...
  private def resolveMetadata(
    ...
    val annotations =
      resolveStringMap(metadata.annotations, "metadata.annotations", variables, None)
  ...
  private def resolveOptionalTarget(
      ...
      variables: VectorMap[String, String]
  ): Resolved[Option[Target]] = target match
    ...
    case Some(value) =>
      val os = value.os match
        ...
        case Some(targetOs) =>
          val family =
            resolveOptionalString(targetOs.family, "spec.target.os.family", variables, None)
          val distribution = resolveOptionalString(
          ...
          val version =
            resolveOptionalString(targetOs.version, "spec.target.os.version", variables, None)
          val codename =
            resolveOptionalString(targetOs.codename, "spec.target.os.codename", variables, None)
          val architecture = resolveOptionalString(
            targetOs.architecture,
            "spec.target.os.architecture",
            variables,
            None
          )
          val desktop =
            resolveOptionalString(targetOs.desktop, "spec.target.os.desktop", variables, None)
          ...
          Resolved(
            ...
            family.errors ++ distribution.errors ++ version.errors ++ codename.errors ++
              architecture.errors ++ desktop.errors
          )
  ...
  private def resolveOptionalSources(
      ...
      variables: VectorMap[String, String]
  ): Resolved[Option[Sources]] = sources match
    case None        => Resolved(None, Vector.empty)
    case Some(value) =>
        ...
        case Some(zypperSources) =>
          val repositories = collect(
            zypperSources.repositories.zipWithIndex.map { case (repository, index) =>
              resolveZypperRepository(
                repository,
                s"spec.sources.zypper.repositories[$index]",
                variables
              )
            }
          )
  ...
  private def resolveDnfRepository(
    ...
    val name        = resolveString(repository.name, s"$at.name", variables, None)
    val description =
      resolveOptionalString(repository.description, s"$at.description", variables, None)
  ...
  private def resolvePlan(
      ...
      variables: VectorMap[String, String]
  ): Resolved[Vector[PlanEntry]] = collect(
    plan.zipWithIndex.map { case (entry, index) => resolvePlanEntry(entry, index, variables) }
  )
  ...
  private def resolvePlanEntry(
    ...
    val description =
      resolveOptionalString(entry.description, s"$at.description", variables, context)
    ...
    Resolved(
      ...
      name.errors ++ kind.errors ++ description.errors ++ execution.errors ++ condition.errors ++
        spec.errors
    )
  ...
  ): Resolved[Option[Execution]] = execution match
    case None        => Resolved(None, Vector.empty)
    case Some(value) =>
  ...
  private def resolveOptionalCondition(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[Option[Condition]] = condition match
    case None        => Resolved(None, Vector.empty)
    case Some(value) =>
      val commandExists =
        resolveOptionalString(value.commandExists, s"$at.commandExists", variables, context)
  ...
  private def resolveOptionalOsCondition(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[Option[OsCondition]] = os match
    case None        => Resolved(None, Vector.empty)
    case Some(value) =>
      val family = resolveOptionalMatchExpression(value.family, s"$at.family", variables, context)
      val distribution =
        resolveOptionalMatchExpression(value.distribution, s"$at.distribution", variables, context)
      val version =
        resolveOptionalMatchExpression(value.version, s"$at.version", variables, context)
      val codename =
        resolveOptionalMatchExpression(value.codename, s"$at.codename", variables, context)
      val architecture =
        resolveOptionalMatchExpression(value.architecture, s"$at.architecture", variables, context)
      val desktop =
        resolveOptionalMatchExpression(value.desktop, s"$at.desktop", variables, context)
      val raw = resolveRaw(value.raw, at, variables, context)
      ...
      Resolved(
        ...
        family.errors ++ distribution.errors ++ version.errors ++ codename.errors ++
          architecture.errors ++ desktop.errors ++ raw.errors
      )
  ...
  private def resolveOptionalMatchExpression(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[Option[MatchExpression]] = expression match
    case None                               => Resolved(None, Vector.empty)
    ...
    case Some(MatchExpression.OneOf(values)) =>
      resolveStringVector(values, s"$at.oneOf", variables, context).map(value =>
        Some(MatchExpression.OneOf(value))
      )
  ...
  private def resolveRaw(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[RawYaml] = raw match
    case RawYaml.StringValue(value) =>
      resolveString(value, at, variables, context).map(RawYaml.StringValue(_))
    case RawYaml.SequenceValue(items) => collect(items.zipWithIndex.map((item, index) =>
        resolveRaw(item, s"$at[$index]", variables, context)
      ))
  ...
  private def resolveStringMap(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[VectorMap[String, String]] = collect(
  ...
  private def resolveStringVector(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[Vector[String]] = collect(values.zipWithIndex.map((value, index) =>
    resolveString(value, s"$at[$index]", variables, context)
  ))
  ...
  private def resolveOptionalString(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[Option[String]] = value match
    case None        => Resolved(None, Vector.empty)
  ...
  private def resolveString(
      ...
      variables: VectorMap[String, String],
      context: Option[String]
  ): Resolved[String] = interpolate(value, at, context): name =>
  ...
  private def resolveSpecVariables(
    ...
    def resolveSpecVariable(
        name: String,
        stack: Vector[String]
    ): Either[Vector[ManifestValidationError], String] = resolved.get(name) match
      case Some(value)                   => Right(value)
      case None if failed.contains(name) =>
        Left(Vector(ManifestValidationError(
          s"spec.vars.$name",
          s"could not resolve variable '$name'"
        )))
      case None => vars.get(name) match
          case None => baseVariables.get(name) match
              case Some(value) => Right(value)
              case None        => Left(Vector(unresolvedVariable(
                  s"spec.vars.${stack.lastOption.getOrElse(name)}",
                  name,
                  None
                )))
          case Some(_) if stack.contains(name) =>
            ...
            Left(Vector(ManifestValidationError(
              s"spec.vars.$name",
              s"cyclic variable reference '$cycle'"
            )))
          case Some(rawValue) =>
    ...
    val errors = vars.keys.toVector.flatMap: name =>
      resolveSpecVariable(name, Vector.empty) match
        case Right(_)     => Vector.empty
        case Left(errors) => errors

    SpecVariableResolution(
      VectorMap.from(vars.keys.toVector.flatMap(name => resolved.get(name).map(name -> _))),
      errors
    )
  ...
  private def collect[A](values: Vector[Resolved[A]]): Resolved[Vector[A]] = Resolved(
    values.map(_.value),
    values.flatMap(_.errors)
  )

  private final case class Resolved[A](value: A, errors: Vector[ManifestValidationError]):
    def map[B](transform: A => B): Resolved[B] = Resolved(transform(value), errors)
...
object RuntimeVariables:
  val empty: RuntimeVariables = RuntimeVariables(VectorMap.empty)

  def from(values: (String, String)*): RuntimeVariables = RuntimeVariables(VectorMap.from(values))

  def fromMap(values: Map[String, String]): RuntimeVariables =
    RuntimeVariables(VectorMap.from(values.toVector))

  def fromHostSystem(system: HostSystem = HostSystem.live): RuntimeVariables = RuntimeVariables(
    VectorMap.from(
