      ...
      case Some(firstFailure) =>
        PlanOperationOutcome.Failed(
          PlanFailure(
            ...
            message = s"${failures.size} shell script item(s) failed; first failure: ${firstFailure.message}",
            exitCode = firstFailure.exitCode
          )
        )
      case None =>
        PlanOperationOutcome.Completed(Vector(completionDetail(ran, skipped)))

  private def runItem(item: ShellScriptItem): Either[ShellScriptFailure, Unit] =
    files.createTempScript(item.name) match
      case Left(error) =>
        Left(ShellScriptFailure.TempFile(item.name, error.message))
      case Right(tempPath) =>
        try downloadAndRun(item, tempPath)
        finally files.deleteIfExists(tempPath)

  private def downloadAndRun(item: ShellScriptItem, tempPath: Path): Either[ShellScriptFailure, Unit] =
    downloader.download(item.url, tempPath) match
      case Left(error) =>
        Left(ShellScriptFailure.Download(item.name, error.message))
      case Right(()) =>
...
private enum ShellScriptFailure:
  ...
  def message: String =
    this match
      case TempFile(itemName, detail) =>
        s"$itemName temp file failed: $detail"
...
object ShellScriptFiles:
  val Jvm: ShellScriptFiles =
    new ShellScriptFiles:
      ...
      override def createTempScript(itemName: String): Either[ShellScriptFileError, Path] =
        try Right(Files.createTempFile("initkit-" + safeName(itemName) + "-", ".sh"))
          ...
          case error: IOException =>
            Left(ShellScriptFileError(error.getMessage))
  ...
  private def dryRunActions(item: ShellScriptItem, files: ShellScriptFiles): Vector[DryRunAction] =
    item.creates match
      case Some(path) if files.exists(Path.of(path)) =>
        Vector(DryRunAction.Message(s"skip shell script '${item.name}': creates path already exists: $path"))
      case _ =>
        val tempPath = files.previewTempScriptPath(item.name)
        Vector(
          DryRunAction.Message(s"download shell script '${item.name}' from ${item.url} to $tempPath"),
