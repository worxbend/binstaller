...
final class DotfilesExecutor(
  ...
  def install(
    ...
    case ExecutionRunMode.Apply => applyDotfiles(operation)

  private def applyDotfiles(
      operation: InstallerPlanOperation[InstallerSpec.DotfilesApply]
  ): PlanOperationOutcome = runSetupCommands(operation) match
    case Left(failure)       => failed(operation.summary, failure)
    case Right(setupResults) => verifyConfig(operation.spec.config) match
        case Left(failure) => failed(operation.summary, failure)
        case Right(())     => runToolCommands(operation) match
            case Left(failure)      => failed(operation.summary, failure)
            case Right(toolResults) => PlanOperationOutcome.Completed(Vector(completionDetail(
                setupResults.size,
                toolResults.size
              )))
  ...
  private def runSetupCommands(
    ...
    runCommands(DotfilesExecutor.setupCommandSpecs(
      operation.spec,
      includeClone = !repositoryExists(operation.spec.repository)
    ))
  ...
  private def runCommands(commands: Vector[CommandSpec])
      : Either[DotfilesFailure, Vector[CommandResult]] =
...
private enum DotfilesFailure:
  ...
  def message: String = this match
    case Command(result) => s"dotfiles command failed: ${DotfilesExecutor.describe(result.spec)} " +
...
object DotfilesFiles:
  ...
  val Jvm: DotfilesFiles = new DotfilesFiles:
...
object DotfilesExecutor:
  ...
  def commandSpecs(operation: InstallerPlanOperation[InstallerSpec.DotfilesApply])
      : Vector[CommandSpec] = setupCommandSpecs(operation.spec, includeClone = true) ++
    toolCommandSpecs(operation.spec)

  def setupCommandSpecs(
      spec: InstallerSpec.DotfilesApply,
      includeClone: Boolean
  ): Vector[CommandSpec] = Option.when(includeClone)(cloneCommandSpec(spec.repository)).toVector ++
  ...
  def dryRunData(operation: InstallerPlanOperation[InstallerSpec.DotfilesApply])
      : DryRunOperationData = DryRunOperationData(
    operation = operation.summary,
    actions = commandSpecs(operation).map(dryRunCommand) :+
      DryRunAction.Message(
        s"verify dotfiles config exists at ${operation.spec.config.path} after checkout"
      )
  )
  ...
  private def direct(values: Vector[String]): CommandSpec = CommandSpec.direct(
