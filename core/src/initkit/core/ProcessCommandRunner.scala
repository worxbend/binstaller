...
final class ProcessCommandRunner(
  ...
  override def run(spec: CommandSpec): CommandResult =
    ...
    mode match
      ...
      case CommandRunMode.Apply =>
        runPrepared(spec, startedAt)

  private def runPrepared(spec: CommandSpec, startedAt: Long): CommandResult =
    sudoStrategy.prepare(spec) match
      case Left(error) =>
        CommandResult(spec, CommandTermination.FailedToStart(error.message), "", "", elapsedSince(startedAt))
      case Right(prepared) =>
        startAndCapture(prepared, startedAt)

  private def startAndCapture(spec: CommandSpec, startedAt: Long): CommandResult =
    commandVector(spec) match
      case Left(message) =>
        CommandResult(spec, CommandTermination.FailedToStart(message), "", "", elapsedSince(startedAt))
      case Right(command) =>
        ...
        try
          ...
          case error: IOException =>
            CommandResult(spec, CommandTermination.FailedToStart(error.getMessage), "", "", elapsedSince(startedAt))
          case error: SecurityException =>
            CommandResult(spec, CommandTermination.FailedToStart(error.getMessage), "", "", elapsedSince(startedAt))

  private def captureProcessResult(spec: CommandSpec, process: Process, startedAt: Long): CommandResult =
    supervised:
  ...
  private def waitForProcess(process: Process, timeout: Option[FiniteDuration]): CommandTermination =
    try
      timeout match
        case Some(duration) =>
        ...
        case None =>
          CommandTermination.Exited(process.waitFor())
    catch
      case error: InterruptedException =>
        terminateProcessTree(process)
        CommandTermination.Cancelled(Option(error.getMessage).getOrElse("interrupted"))
  ...
  private def commandVector(spec: CommandSpec): Either[String, Vector[String]] =
    spec.invocation match
      case CommandInvocation.Direct(argv) if argv.isEmpty =>
        Left("command argv is empty")
      case CommandInvocation.Direct(argv) =>
        Right(argv.map(_.value))
  ...
  private def elapsedSince(startedAt: Long): FiniteDuration =
    (clock.millis() - startedAt).millis
...
private final class BoundedStreamCapture(maxBytes: Int):
  ...
  def append(bytes: Array[Byte], count: Int): Unit =
        ...
        try
          ...
          try
            ...
            case error: InterruptedException =>
              ...
              Left(SudoPreparationError(s"sudo preflight was interrupted: ${Option(error.getMessage).getOrElse("interrupted")}"))
        catch
          case error: IOException =>
...
final class PreflightSudoStrategy(
  ...
  override def prepare(spec: CommandSpec): Either[SudoPreparationError, CommandSpec] =
    spec.sudo match
      ...
      case SudoMode.Required =>
        preflight.validate(SudoPreflightRequest(preflightMode)).map(_ => withSudo(spec, preflightMode))

  private def withSudo(spec: CommandSpec, mode: SudoPreflightMode): CommandSpec =
    spec.invocation match
      case CommandInvocation.Direct(argv) =>
        spec.copy(
          invocation = CommandInvocation.Direct(sudoArguments(mode).map(value => CommandArgument(value)) ++ argv),
  ...
  private def sudoArguments(mode: SudoPreflightMode): Vector[String] =
    mode match
...
object PreflightSudoStrategy:
  ...
  ): Either[SudoPreparationError, PreflightSudoStrategy] =
    interactive match
...
final class FakeSudoPreflight private (
  ...
  def calls: Vector[SudoPreflightRequest] =
    stateRef.get().calls

  override def validate(request: SudoPreflightRequest): Either[SudoPreparationError, Unit] =
    val state = stateRef.get()
    val (result, pending) = state.pending.headOption match
      case Some(response) if response.expected == request =>
        response.result -> state.pending.tail
      case Some(response) =>
        Left(SudoPreparationError(s"unexpected sudo preflight: expected ${response.expected}")) -> state.pending
      case None =>
        Left(SudoPreparationError("unexpected sudo preflight: no fake response configured")) -> Vector.empty
