...
object TuiExecutionTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    test("preview selected uses engine dry-run operation generation"):
      ...
      assert(report.result.events.exists:
        case PlanEvent.DryRunOperation(operation, data, _) => operation.name == "post-install" &&
          data.actions.exists:
            case DryRunAction.Command(argv, shell, _, _, _) => argv == Vector("echo hello") &&
              shell.contains("/bin/sh -c")
            case _ => false
        case _ => false)
    ...
    test("run selected uses shared execution engine and logs command output"):
      ...
      val fakeExecutor = FakeCommandExecutor(
        Vector(FakeCommandResponse(
          command,
          CommandResultData.exited(
            0,
            stdout = "hello\n",
            stderr = "warn\n",
      ...
      val report =
        requireRight(runner.run(fixture.context, fixture.model, TuiExecutionAction.RunSelected))
    ...
    test("apply actions run source setup before package commands"):
      ...
      actions.foreach: action =>
        ...
        assert(report.result.events.collect { case PlanEvent.Completed(operation, _, _) =>
          operation.name
        } == Vector(
          "source-setup",
          "apt-base-cli"
    ...
    test("source setup failure stops TUI package execution"):
      ...
      val fakeExecutor  = FakeCommandExecutor(
        Vector(FakeCommandResponse(
          sourceCommand,
          CommandResultData.exited(5, duration = Duration.Zero)
        ))
      )
      val writer  = RecordingStateWriter()
      val runner  = TuiExecutionRunner(_ => fakeExecutor, writer)
    ...
    test("quit confirmation text is shown while work is running"):
      ...
      assert(TuiTextLayout.statusLine(
        fixture.model,
        TuiWorkStatus.Running("run selected"),
        confirmQuit = true
      ).contains("confirm quit"))
  ...
  private def packageManifest(): Manifest = Manifest(
    apiVersion = Some("initkit.io/v1alpha1"),
    kind = Some("WorkstationProfile"),
    metadata = Metadata(Some("developer-workstation"), VectorMap.empty, VectorMap.empty),
    spec = ManifestSpec(
  ...
  private def aptEntry(name: String): PlanEntry = PlanEntry(
    name = Some(name),
  ...
  private def commandEntry(name: String): PlanEntry = PlanEntry(
    name = Some(name),
    kind = Some("commands"),
  ...
  private def packageCommands(manifest: Manifest): Vector[CommandSpec] =
    PlanOperation.decode(0, manifest.spec.plan.head) match
      case Right(PlanOperation.AptPackages(operation)) => PackageManagerInstallers.commandSpecs(
          operation,
          ExecutionPolicy(
      ...
      case Right(other) => throw new java.lang.AssertionError(s"expected apt operation, got $other")
      case Left(errors) => throw new java.lang.AssertionError(errors.map(_.message).mkString("; "))
  ...
  private final class RecordingStateWriter extends ExecutionStateWriter:
    ...
    def writes: Vector[(Path, ExecutionState)] = recorded
