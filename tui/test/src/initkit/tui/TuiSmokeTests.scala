...
object TuiSmokeTests extends TestSuite:
  ...
  private def smokeConfig(backend: Backend): TuiConfig = TuiConfig
    .builder()
    .backend(backend)
    .rawMode(false)
  ...
  private def exampleConfig: Path = Iterator
...
private final class RecordingBackend private (
  ...
  def drawCount: Int = currentDrawCount.get()

  def closed: Boolean = currentClosed.get()

  override def draw(diff: DiffResult): Unit = currentDrawCount.incrementAndGet()

  override def flush(): Unit = ()

  override def clear(): Unit = ()

  override def size(): Size = Size(120, 40)

  override def showCursor(): Unit = ()

  override def hideCursor(): Unit = ()

  override def getCursorPosition(): Position = Position(0, 0)
  ...
  override def read(timeoutMs: Int): Int = -2

  override def peek(timeoutMs: Int): Int = -2

  override def writeRaw(data: Array[Byte]): Unit = ()

  override def close(): Unit = currentClosed.set(true)

private object RecordingBackend:
  def apply(): RecordingBackend = new RecordingBackend(AtomicInteger(0), AtomicBoolean(false))
