...
object CliColorMode:
  ...
  def parse(value: String): Either[String, CliColorMode] = value.trim.toLowerCase match
...
final class CliRenderer(settings: CliColorSettings):
  def section(value: String): String = styled(value, Color.Cyan.apply)

  def ok(value: String): String = styled(value, Color.Green.apply)

  def skipped(value: String): String = styled(value, Color.Yellow.apply)

  def failed(value: String): String = styled(value, Color.Red.apply)

  def muted(value: String): String = styled(value, Color.DarkGray.apply)

  def label(value: String): String = styled(value, Color.Blue.apply)
...
object ApplyReporter:
  ...
  def debugLines(report: ApplyReport): Vector[String] = Vector(
  ...
  private def printHeader(report: ApplyReport, out: PrintWriter, renderer: CliRenderer): Unit =
    ...
    out.println(
      s"${renderer.label("manifest")}: ${report.manifest.metadata.name.getOrElse("<unnamed>")}"
    )
  ...
  private def printSourceSetup(
    ...
    if plan.operations.isEmpty && plan.skippedSections.isEmpty then
      out.println("  no source setup operations")
    else
      ...
      if plan.aptUpdateBeforeInstall then
        out.println(
          s"  ${renderer.muted("note")} apt package installs will run apt-get update before packages"
        )
  ...
  private def printSelection(
    ...
    if selection.runnable.isEmpty then out.println("  <none>")
      ...
      selection.runnable.foreach: entry =>
        out.println(
          s"  ${renderer.ok("run")} ${entry.entry.name.getOrElse("<unnamed>")} (${entry.entry.kind.getOrElse("<unknown>")})"
        )
    ...
    if selection.skipped.isEmpty then out.println("  <none>")
      ...
      selection.skipped.foreach: entry =>
        val name = entry.entry.name.getOrElse("<unnamed>")
        val kind = entry.entry.kind.getOrElse("<unknown>")
        out.println(
          s"  ${renderer.skipped("skip")} $name ($kind): ${entry.userFacingReasons.mkString("; ")}"
        )

  private def printOperations(
  ...
  private def printDryRunData(
      data: DryRunOperationData,
      out: PrintWriter,
      renderer: CliRenderer
  ): Unit =
  ...
  private def actionLine(action: DryRunAction, renderer: CliRenderer): String = action match
    ...
    case DryRunAction.Message(text) => s"${renderer.muted("note")} $text"
  ...
  private def eventDebug(event: PlanEvent): String = event match
    case PlanEvent.Scheduled(operation, _)        => s"scheduled:${operation.name}"
    case PlanEvent.Started(operation, _)          => s"started:${operation.name}"
  ...
  private def actionDebug(action: DryRunAction): String = action match
    case DryRunAction.Command(argv, shell, sudo, workingDirectory, stdinFile) =>
    ...
    case DryRunAction.Message(text) => s"message $text"
...
final class CliDebugLogger private (
  ...
  def enabled: Boolean = writer.isDefined

  def line(value: String): Unit = writer.foreach: target =>
    target.println(s"${Instant.now()} ${CommandRedactor.redactText(value)}")
    target.flush()

  def lines(values: Iterable[String]): Unit = values.foreach(line)

  def close(): Unit = if closeWriter then writer.foreach(_.close())

object CliDebugLogger:
  def disabled: CliDebugLogger = new CliDebugLogger(None, closeWriter = false)
  ...
  def fromOptions(
    ...
    if !debug && debugLog.isEmpty then Right(disabled)
      ...
      debugLog match
        ...
        case None => Right(new CliDebugLogger(Some(err), closeWriter = false))
