...
archive extraction, installer scripts, symlink creation, state writes, dry-run
fidelity, CLI reporting, tests, and release workflow.
...
- Implemented: Plan output redacts installer environment values. Env names remain
  visible, but values are rendered as `<redacted>` to avoid leaking runtime
  variables or copied secrets.
- Implemented: Installer command failure rendering now quotes argv elements so
  diagnostics preserve argument boundaries and do not imply shell evaluation.
- Deferred with rationale: `tar.xz` extraction still depends on the system `tar`
  ...
  and tar.gz paths.
- Deferred with rationale: installer-script installs still operate in the final
  install directory because the current manifest's upstream installer scripts
  expect `${downloadPath}` and local install layout. Direct binary and native
  archive installs are staged before replacement; script installs should move to
  a staged working directory once installer contract changes are designed.
...
- Consider sandboxed installer-script execution with restricted filesystem and
  network access.
...
- Can manifest values become executable shell syntax? Interpolation treats shell
  ...
  The intentional shell boundaries are installer scripts (`sh`, `bash`, `zsh`)
  and the temporary `tar` fallback for `tar.xz`. Installer script env is explicit
  and no parent env is inherited except baseline `PATH`.
- Are command args preserved as argv? Yes for installer scripts, sudo symlinks,
  and `tar.xz`; dry-run now quotes args for display and failure output quotes
  argv elements.
...
- Are downloads atomic and verified before replacement? Direct binary and native
  ...
  then replace. Installer-script installs verify checksum before execution but
  are not staged atomically.
...
- Can installer scripts unexpectedly use sudo or inherit unsafe env? Sudo is not
  introduced by installer fields; scripts can execute whatever their script body
  contains, so script provenance remains a trust boundary. Parent env is cleared
  and only baseline `PATH` plus declared env is passed.
- Can installer scripts read sensitive parent env vars? Not through the process
  environment. They may still read files accessible to the user because no
  sandbox is present.
- Do installer scripts have timeout/cancellation and cleanup behavior? Process
  execution now has a timeout. Downloaded script cleanup is best effort and
  reported as a failure when configured cleanup fails.
...
- Are error messages actionable without exposing secrets? Validation and apply
  failures include paths/actions. Installer env values are now redacted in plan
  output. Some filesystem and process errors still include paths by design.
- Are dry-run and apply close enough? Plan/apply dry-run share resolution,
  selection, and rendering. Apply has extra preflight and executor phases. Script
  install staging and tar.xz inspection are the main remaining divergences.
- Do tests cover real failure modes? Tests cover invalid manifests, URL scheme,
  ...
  mismatch, redaction, timeout, stale state, continue-on-error, and dry-run
