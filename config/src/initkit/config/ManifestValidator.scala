...
object ManifestValidator:
  ...
  private val SupportedPlanKinds = PackageSpecDecoder.PackageKinds ++
    InstallerSpecDecoder.InstallerKinds

  def validate(
      manifest: Manifest,
      manifestPath: Option[Path] = None
  ): Either[Vector[ManifestValidationError], Manifest] =
    val errors = validateTopLevel(manifest) ++
  ...
  private def validateKindSpecificSpec(
    ...
    entry.spec match
      ...
      case Some(RawYaml.MappingValue(_)) => kind match
          case packageKind if PackageSpecDecoder.isPackageKind(packageKind) =>
            PackageSpecDecoder.decode(
              packageKind,
              entry.spec,
              specAt,
              entry.name
            ).left.toOption.getOrElse(Vector.empty)
  ...
  private def validateExecution(
      ...
      at: String
  ): Vector[ManifestValidationError] = execution match
  ...
  private def validateExecutionMode(
      mode: Option[String],
      at: String
  ): Vector[ManifestValidationError] = mode match
    case None                         => Vector.empty
  ...
  private def validateMaxConcurrency(
      ...
      mode: String,
      at: String
  ): Vector[ManifestValidationError] = value match
  ...
  private def validateDuplicatePlanNames(plan: Vector[PlanEntry]): Vector[ManifestValidationError] =
    ...
    namedEntries
      .foldLeft((Map.empty[String, Int], Vector.empty[ManifestValidationError])) {
        case ((seen, errors), (name, index)) => seen.get(name) match
            case Some(firstIndex) =>
              val detail =
                s"duplicate plan name '$name' also used at ${planPath(firstIndex, "name")}"
  ...
  private def validateRequiredFixedValue(
      ...
      expected: String,
      at: String
  ): Vector[ManifestValidationError] = value.flatMap(nonEmptyTrimmed) match
    case None                               => Vector(error(at, s"$fieldName is required"))
    case Some(actual) if actual == expected => Vector.empty
    case Some(actual)                       =>
      Vector(error(at, s"unsupported $fieldName '$actual'; expected '$expected'"))
